 ...
 */
static const char* BITCOIN_PID_FILENAME = "terrydaviscoind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("terrydaviscoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/acejr1337/TerryDavisCoin>";
    const std::string URL_WEBSITE = "<https://terrydaviscoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("terrydaviscoin-loadblk");
...
//        LogPrintf("Config file: %s (not found, skipping)\n", config_file_path.string());
        ...
        if (configFile != NULL) {
            std::string strHeader = "rpcuser=username\n"
                                    ...
                                    "addnode=167.71.212.80:9553\n"
                                    "addnode=206.189.226.157:9553\n"
                                    "addnode=140.82.1.236:9553\n";
...
//        return; // Nothing to read, so just return
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if terrydaviscoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if terrydaviscoin is started while in a temporary directory.\n",
